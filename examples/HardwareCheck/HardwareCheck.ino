...
static constexpr auto BOARD = CameraSetup::Board::EspEye;
...
void setup() {
    ...
    if (!err) {
        for (int i=0; i<10; ++i) {
            ...
            Serial.printf("CHECK JPEG %d %s %u bytes\n", i,
                f && f->len > 2 && f->buf[0] == 0xff && f->buf[1] == 0xd8 ? "PASS" : "FAIL", f ? f->len : 0);
    ...
    if (!ret) Serial.printf("CHECK PEER start %d\n", rtc.startConnection());
    start = millis();
    while (rtc.handle() && millis()-start < 3000) { rtc.loop(); delay(1); }
    rtc.end();
    Serial.printf("CHECK FINISHED SDP=%d heap=%u PSRAM-free=%u\n", gotSdp, ESP.getFreeHeap(), ESP.getFreePsram());
}

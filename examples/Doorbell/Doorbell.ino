...
#elif DOORBELL_BOARD == BOARD_WROVER_KIT
    return CameraSetup::Board::WroverKit;
#elif DOORBELL_BOARD == BOARD_ESP32S3_WROOM
    return CameraSetup::Board::Esp32S3Wroom;
#elif DOORBELL_BOARD == BOARD_ESP32S3_GOOUUU
    return CameraSetup::Board::Esp32S3Goouuu;
#elif DOORBELL_BOARD == BOARD_LILYGO_CAMERA
    return CameraSetup::Board::LilygoCamera;
#else
#error "Unknown DOORBELL_BOARD; choose a profile from BoardProfiles.h."
#endif
}
...
void stopSession() {
    ...
    rtc.end();
    char *msg;
    while (xQueueReceive(outgoing, &msg, 0) == pdTRUE)
        free(msg);
...
int onSignal(esp_peer_msg_t *msg, void *) {
    ...
    memcpy(value + 1, msg->data, msg->size);
    value[msg->size + 1] = 0;
...
int onData(esp_peer_data_frame_t *f, void *) {
    if (f->type != ESP_PEER_DATA_CHANNEL_STRING || f->size < 1 || f->size > 64)
        return 0;
    String command;
    command.concat(reinterpret_cast<const char *>(f->data), f->size);
    if (command == "ACCEPT_CALL")
        streaming = true;
    else if (command == "DENY_CALL")
        streaming = false;
...
bool authorized() {
    if (server.header("X-Viewer-Token") == VIEWER_TOKEN)
        return true;
    server.send(403, "text/plain", "Invalid viewer token");
    return false;
}
...
void configureServer() {
    ...
    server.on("/offer", HTTP_POST, [] {
        ...
        if (sdp.length() > 16384 || !sdp.startsWith("v=0")) {
            server.send(400, "text/plain", "Invalid SDP");
            return;
        }

        if (sessionActive.exchange(true)) {
            server.send(409, "text/plain", "Viewer already connected; disconnect it first");
            return;
        }
        ...
        if (!enqueue(Action::Start, sdp)) {
            sessionActive = false;
            server.send(503, "text/plain", "Peer queue full");
            return;
        }
    ...
    server.on("/signal", HTTP_GET, [] {
        ...
        if (xQueueReceive(outgoing, &msg, 0) == pdTRUE) {
            ...
            server.send(410, "text/plain", "Peer session closed");
        else
            server.send(204);
    });
    server.on("/candidate", HTTP_POST, [] {
        ...
        if (value.length() > 2048 || !value.startsWith("candidate:")) {
            server.send(400, "text/plain", "Invalid candidate");
            return;
        }
...
void cameraLoop() {
    ...
    if (!frame) {
        if (millis() - lastFrame < 200)
            return; // At most 5 fps initially.
        lastFrame = millis();
        frame = esp_camera_fb_get();
        if (!frame)
            return;
        if (frame->format != PIXFORMAT_JPEG || frame->len > 128 * 1024) {
            releaseFrame();
            return;
        }
    ...
    uint8_t packet[1040];
    uint32_t header[] = {0x47504A53, frameId, static_cast<uint32_t>(frame->len),
    ...
    if (ret == 0)
        frameOffset += bytes;
    if (frameOffset == frame->len || millis() - frameStarted > 1000 ||
...
void audioLoop() {
    ...
    i2s_channel_read(microphone.rxChan(), reinterpret_cast<uint8_t *>(pcm) + used,
                     sizeof(pcm) - used, &bytes, 0);
    used += bytes;
    if (used != sizeof(pcm))
        return;
    used = 0;
    ...
    while (true) {
        ...
        while (xQueueReceive(commands, &cmd, 0) == pdTRUE) {
            if (cmd.action == Action::Stop)
                stopSession();
            else if (cmd.action == Action::Ring && channelReady)
                rtc.sendText(channelId, "RING");
            else if (cmd.action == Action::Candidate && rtc.handle()) {
                rtc.signal(ESP_PEER_MSG_TYPE_CANDIDATE, reinterpret_cast<uint8_t *>(cmd.data),
                           cmd.size);
                ...
                cfg.on_channel_open = onChannel;
                cfg.on_channel_close = onChannelClosed;
                cfg.on_data = onData;
#if DOORBELL_MIC
                ...
                if (!ret)
                    ret = rtc.startConnection();
                if (!ret)
                    ret = rtc.signal(ESP_PEER_MSG_TYPE_SDP, reinterpret_cast<uint8_t *>(cmd.data),
                                     cmd.size);
                if (ret) {
                    Serial.printf("Peer start failed: %d\n", ret);
                    closeRequested = true;
                }
            }
        ...
        if (rtc.handle())
            rtc.loop();
        if (closeRequested || (rtc.handle() && millis() - lastActivity.load() > 15000))
            stopSession();
        audioLoop();
        cameraLoop();
        vTaskDelay(1);
...
void setup() {
    ...
    if (RING_BUTTON_PIN >= 0)
        pinMode(RING_BUTTON_PIN, INPUT_PULLUP);
    ...
    Serial.printf("Camera profile: %s; PSRAM: %u bytes\n", CameraSetup::name(cameraBoard()),
                  ESP.getPsramSize());
    Serial.printf("Camera pins: XCLK=%d SDA=%d SCL=%d PWDN=%d RESET=%d\n", cameraConfig.pin_xclk,
                  cameraConfig.pin_sccb_sda, cameraConfig.pin_sccb_scl, cameraConfig.pin_pwdn,
                  cameraConfig.pin_reset);
    esp_err_t cameraResult = WebRTCCamera::begin(cameraConfig);
    if (cameraResult != ESP_OK) {
        Serial.printf("Camera failed: 0x%x (%s).\n", cameraResult, esp_err_to_name(cameraResult));
        Serial.println("Check CameraConfig.h pins, ribbon connection, power, and PSRAM settings.");
        Serial.println("For AI-Thinker ESP32-CAM, select BOARD_AI_THINKER, not BOARD_ESP_EYE.");
        while (true)
            delay(1000);
    }
...
#error "This example's onboard microphone profile is XIAO ESP32S3 Sense only."
    ...
    if (!commands || !outgoing ||
        xTaskCreate(peerTask, "webrtc", 24 * 1024, nullptr, 4, nullptr) != pdPASS) {
        Serial.println("Not enough memory for WebRTC task/queues");
        while (true)
            delay(1000);
    }

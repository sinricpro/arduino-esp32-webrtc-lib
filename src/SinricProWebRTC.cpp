...
int SinricWebRTC::begin(esp_peer_cfg_t cfg) {
...
void SinricWebRTC::end() {
    if (peer_) { esp_peer_close(peer_); peer_ = nullptr; }
}
int SinricWebRTC::loop() { return peer_ ? esp_peer_main_loop(peer_) : ESP_PEER_ERR_WRONG_STATE; }
int SinricWebRTC::startConnection() { return peer_ ? esp_peer_new_connection(peer_) : ESP_PEER_ERR_WRONG_STATE; }
int SinricWebRTC::signal(esp_peer_msg_type_t type, const uint8_t *data, size_t size) {
    if (!peer_) return ESP_PEER_ERR_WRONG_STATE;
...
int SinricWebRTC::sendBinary(uint16_t channel, const uint8_t *data, size_t size) {
    if (!peer_) return ESP_PEER_ERR_WRONG_STATE;
...
int SinricWebRTC::sendText(uint16_t channel, const char *text) {
    if (!peer_) return ESP_PEER_ERR_WRONG_STATE;
